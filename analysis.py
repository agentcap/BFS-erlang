...
rp = [1, 1, 2, 2, 2, 10, 10, 10]
cp = [1, 2, 2, 3, 4, 10, 5, 8, 10]
proc = [1, 2, 4, 6, 8, 80, 100]
...
# for i in range(5):
	...
	os.system('cd sequential && erlc *.erl && erl -noshell -s bfs main ../analysis/inpSeqMeta ../analysis/inpSeqAdj time -s init stop > ../analysis/time.txt && cd ..')
	os.system('cd shared && erlc *.erl && erl -noshell -s bfs_shared main ../analysis/inpSrdMeta ../analysis/inpSrdAdj time -s init stop >> ../analysis/time.txt && cd ..')
	os.system('cd 1D && erlc *.erl && erl -noshell -s bfs_1d main ../analysis/inp1D ../analysis time -s init stop >> ../analysis/time.txt && cd ..')
	os.system('cd 2D && erlc *.erl && erl -noshell -s bfs_2d main ../analysis/inp2D ../analysis time -s init stop >> ../analysis/time.txt && cd ..')
	with open('analysis/time.txt', 'r') as f:
		times = f.readlines()
		tseq.append(float(times[0].strip('\n').split(" ")[1]))
		tsrd.append(float(times[1].strip('\n').split(" ")[1]))
		t1d.append(float(times[2].strip('\n').split(" ")[1]))
		t2d.append(float(times[3].strip('\n').split(" ")[1]))
